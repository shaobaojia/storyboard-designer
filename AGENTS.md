...
## 刚做完（rename_seq 场景层预检 + queue.py 拆分 + CSS 2255 行拆分 + v0.9.72 一项 + 屎山治理批 A + v0.9.70 一项 + v0.9.69 一项 + v0.9.68 一项 + v0.9.67 一项 + v0.9.66 一项 + v0.9.65 一项 + v0.9.64 五项，2026-08-11/10，已推 v0.9.73）

**rename_seq 场景层预检（2026-08-11，未推，v0.9.74）**：批量重命名候选名撞孤儿场景（DB 无记录但场景层有 Shot_c0010）→ cmd_rename_shot 第二阶段防撞 raise → 镜头停 __ren_ 前缀（坑 467 修复）。**改动**：①commands_shots.py 新增只读命令 cmd_list_scene_names（返回 bpy.data.scenes.keys()）②queue.py COMMANDS 注册（list_scene_names）③actions.py 新增 _query_scene_names（queue_command callback + threading.Event 同步查询，10s 超时返回 None 保守回退维持旧行为）+ _queue 透传 callback ④_batch_rename_seq 候选名同时避开 DB 重名与场景层占用（occupied = Shot_ 前缀场景名去前缀，查询失败时跳过检查）。**验证**：孤儿场景实锤（场景层有 Shot_c0010、DB 无 c0010）→ MCP 直调 cmd_rename_shot 零副作用复现 raise "Scene Shot_c0010 already exists" → 真实链路（临时镜头 c0012 批量重命名：跳过 c0010 场景占用 + c0020/c0030 DB 占用，落空闲 c0040，DB/场景/相机四层一致，errors=[]）→ 临时镜头软删+purge 全清（DB/垃圾桶/场景/目录）→ 防撞保护回归仍 raise。版本号不升

**审计 MCP 并发根因修复（2026-08-11，推前全量必崩 4/4 → 修复后 89/89）**：**根因** = 本机 BlenderMCP_addon.py 是 patch 版（"0.0.0.0+绕timer"），_handle_client 的 PATCHED 注释明说绕过主线程 timer 直接执行——**所有命令（execute_code 等）在 socket 线程直跑**；审计全量 40+ 次 MCP 调用（state()/blender() 遍历 bpy.data）与主线程 timer 渲染并发 = Blender 不支持的并发 → 堆损坏（python311.dll ACCESS_VIOLATION / render.opengl 挂起 / HTTP 线程 ntpath 崩 = 同根因三症状）。**修复**：BlenderMCP_addon.py 加主线程执行泵（start() 主线程注册 persistent timer `_pump`，socket 线程只往 queue.Queue put + Event wait，零 bpy 接触；execute_code 重写走泵，返回结构不变）。audit_lib.blender() 保持直调（**勿加 timer 包装**——包装代码的 register 在 socket 线程执行 = BKE_timer_add 无锁链表新竞态，曾误改又还原）。**验证**：修复前全量 4/4 崩（20:51/20:53/21:00/23:07，崩点随机）；修复后 89/89 全 PASS（47+12+30）。**遗留观察**：20Hz 极端压力（MCP 遍历 + 渲染）下仍崩（CT_040）——audit 真实节奏 0.2Hz 安全；极端高频 timer+渲染是 Blender 4.5.5 深层问题，不在本轮范围。⚠️ **BlenderMCP 改动只在本机部署目录**（不在 git 仓库）——换机/重装需重打补丁，补丁内容见交接笔记 2026-08-11 晚
...
### 近期（v0.9.72，未推）
- **正式工程连续渲染后 OpenGL 挂起**（2026-08-11 全量审计实测，**根因已定位 = 审计 MCP 并发，见刚做完「审计 MCP 并发根因修复」**）：bpy.ops.render.opengl(write_still=True) 在 timer 主线程调用后**不返回**（主线程栈可见卡在 _op_call）→ timer 停摆 → queue 积压（qsize 可达 50+）→ API 线程饿死假死（先 timeout 后 ConnectionRefused）。**旧判定"大场景触发/干净库不触发/与代码无关"已被推翻**：当晚干净库连崩 4/4（python311.dll ACCESS_VIOLATION，崩点随机），根因 = 本机 patch 版 BlenderMCP 所有命令在 socket 线程直跑（PATCHED 绕 timer），审计 40+ 次 MCP 遍历与主线程渲染并发 = 堆损坏。修复 = BlenderMCP execute_code 主线程泵，修复后 89/89。**保留**：全量审计只在干净库跑（推前标准姿势），正式工程只跑段级 --only；挂起后重启 Blender 恢复。诊断手法：MCP 查 sys._current_frames()[主线程].stack 定位卡点 + qsize 积压确认。
- **rename_seq 候选名只查 DB 不查场景**（2026-08-11 实测，**v0.9.74 已修复**）：_batch_rename_seq 的 name_exists 只查 shots 表 name，cmd_rename_shot 有场景层冲突保护（`Shot_{name} in bpy.data.scenes` raise）——正式工程有**残留场景 Shot_c0010**（DB 无记录，历史删除不彻底）时批量改名候选名撞场景 → rename_shot 报 "Scene Shot_c0010 already exists" → 该镜头停在 __ren_ 前缀（两阶段改名 phase2 失败）。修复 = 候选名生成前同步查场景名集合（新命令 list_scene_names + actions 层 _query_scene_names，Event 等主线程回调，10s 超时保守回退），occupied 场景名跳过。**补充发现：get_other_scenes 只查 DB（origin='other'），孤儿场景在 sync 登记前网页「其它」视图看不到**——治理残留场景只能靠 Blender 手动删或 sync 后处理
