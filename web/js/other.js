...
export async function enterOtherMode() {
    ...
    // v0.9.61：其它视图四功能按钮（时间线/预览/创建/台词）统一灰掉+去蓝底——
    // syncViewToggleButton 是必经入口（原 v0.9.27/58/60 散落的显式 classList 收编到此）
    syncViewToggleButton();
...
export async function exitOtherMode(silent) {
    ...
    // v0.9.61：退出其它视图恢复四按钮（syncViewToggleButton 按状态还原蓝底）
    syncViewToggleButton();
    updateStats();
